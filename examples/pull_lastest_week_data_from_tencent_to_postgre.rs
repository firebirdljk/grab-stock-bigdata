...
struct WeeklyData {
     code:Option<String>,
     name:Option<String>,
     week_start_pri:Option<String>,
     week_end_pri:Option<String>,
     week_max:Option<String>,
     week_min:Option<String>,
     date:Option<String>,
}
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in selected_stock_bases{
        ...
        let mut web_url="http://data.gtimg.cn/flashdata/hushen/weekly/".to_string()+&code+&end_Url;
        println!("{}",web_url);
        ...
        loop {
            ...
            let tmp_code=&code;
            let tmp_name=&name;
            let data=WeeklyData{code:Some(tmp_code.into()),name:Some(tmp_name.into()),week_start_pri:Some(month_start_pri.unwrap().to_string().into()),
                week_end_pri: Some(month_end_pri.unwrap().to_string().into()),
                week_max:Some(month_max.unwrap().to_string().into()),
                week_min:Some(month_min.unwrap().to_string().into()),date:Some(date.unwrap().to_string().into())};
            //save_monthly_data(data);
            let mut tx = conn.start_transaction(mto::default()).unwrap();
            let sql = "INSERT INTO firebird_stock.weekly_data(code,name,weekStartPri,weekEndPri,weekMax,weekMin,date) VALUES (?,?,?,?,?,?,?)";
            let sel_Sql:String="SELECT code from firebird_stock.weekly_data where code='".to_string();
            ...
            if is_ok==0 {
                tx.exec_drop(sql, (stock_code, data.name.unwrap(), data.week_start_pri.unwrap(),
                                   data.week_end_pri.unwrap(), data.week_max.unwrap(), data.week_min.unwrap(),
                                   stock_date, )).unwrap();
                tx.commit();
            }
        }
